...
class GamePlot:
    ...
    def Plot(self, save=False, AltColor=[False, False]):
        ...
        plt.legend(loc=2)
        plt.grid(b=True, axis='x',color='#B2B2B2')             
...
class GameMap():
    ...
    def Map(self, save=False, AltColor=[False, False], shots=True):
        ...
        def VLines():
            ...
            plt.axvline(x=-89, ymin=ymin, ymax=ymax,c='r',lw=2) # Add first goal line
            ...
            plt.axvline(x=89, ymin=ymin, ymax=ymax,c='r', lw=2)       # Add second goal line
        ...
        #### Shade extra white space on rink plot for board curves
        xs = [i/100 for i in range(7375,10001,10)]
        xs = pd.Series(xs)
        ys = [((x-73.75)**5 / (26.25**4))-42.5 for x in xs]
        ys = pd.Series(ys)
        
        ax.fill_between(xs, -42.5, ys, color=Away_Color)
        ax.fill_between(xs, -ys, 42.5, color=Away_Color)

        ax.fill_between(-xs, -42.5, ys, color=Home_Color)
        ax.fill_between(-xs, -ys, 42.5, color=Home_Color)
        ...
        ax.annotate(f"{Home} Attacks",xy=(0,-0.25),xycoords='axes fraction', xytext=(-0.015, 0.4), fontsize=20, rotation = 90) 
        ax.annotate(f"{Away} Attacks",xy=(0,-0.1),xycoords='axes fraction', xytext=(1.00, 0.4), fontsize=20, rotation = -90) 
